...
contains
    ...
    module subroutine compute_gradient_shots_tloc(phase_label)
        ...
        if (.not. any(model_name == 'sx')) then
            sx(:, 1) = gmtr(shot_in_rank(rankid, 1))%recr(:)%x
        end if

        sz = zeros(nr_virtual, 1)
        if (.not. any(model_name == 'sz')) then
            sz(:, 1) = gmtr(shot_in_rank(rankid, 1))%recr(:)%z
        end if

        st0 = zeros(nr_virtual, 1)
        if (.not. any(model_name == 'st0')) then
            st0(:, 1) = gmtr(shot_in_rank(rankid, 1))%recr(:)%t0
        end if
        ...
        do ishot = shot_in_rank(rankid, 1), shot_in_rank(rankid, 2)
            ...
            do i = 1, gmtr(ishot)%nr
                if (any(model_name == 'sx')) then
                    gmtr(ishot)%recr(i)%x = sx(i, 1)
                end if
                if (any(model_name == 'sz')) then
                    gmtr(ishot)%recr(i)%z = sz(i, 1)
                end if
                if (any(model_name == 'st0')) then
                    gmtr(ishot)%recr(i)%t0 = st0(i, 1)
                end if
            end do
                ...
                case ('acoustic-iso')
                    ...
                    do i = 1, nmodel
                        ...
                        if (model_name(i) == 'vp') then
                            ...
                            !$omp parallel do private(j, irx, irz, dxt, dzt)
                            do j = 1, gmtr(ishot)%nr
                                irx = nint((sx(j, 1) - shot_xbeg)/dx) + 1
                                irz = nint((sz(j, 1) - shot_zbeg)/dz) + 1
                                dxt = (ttp(irz, clip(irx + 1, 1, shot_nx)) - ttp(irz, clip(irx - 1, 1, shot_nx)))/(2*dx)
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + ttp_residual(j, 1)*dxt
                            end do
                            !$omp end parallel do
                            ...
                            do j = 1, gmtr(ishot)%nr
                                irx = nint((sx(j, 1) - shot_xbeg)/dx) + 1
                                irz = nint((sz(j, 1) - shot_zbeg)/dz) + 1
                                dzt = (ttp(clip(irz + 1, 1, shot_nz), irx) - ttp(clip(irz - 1, 1, shot_nz), irx))/(2*dz)
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + ttp_residual(j, 1)*dzt
                            end do
                            !$omp end parallel do
                            ...
                            do j = 1, gmtr(ishot)%nr
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + ttp_residual(j, 1)
                            end do
                            !$omp end parallel do
                    ...
                    do i = 1, nmodel
                        ...
                        if (model_name(i) == 'vp') then
                            ...
                            do j = 1, gmtr(ishot)%nr
                                ...
                                dxt = (ttp(irz, clip(irx + 1, 1, shot_nx)) - ttp(irz, clip(irx - 1, 1, shot_nx)))/(2*dx)
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + ttp_residual(j, 1)*dxt
                                dxt = (tts(irz, clip(irx + 1, 1, shot_nx)) - tts(irz, clip(irx - 1, 1, shot_nx)))/(2*dx)
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + tts_residual(j, 1)*dxt
                            end do
                            !$omp end parallel do
                            ...
                            do j = 1, gmtr(ishot)%nr
                                ...
                                dzt = (ttp(clip(irz + 1, 1, shot_nz), irx) - ttp(clip(irz - 1, 1, shot_nz), irx))/(2*dz)
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + ttp_residual(j, 1)*dzt
                                dzt = (tts(clip(irz + 1, 1, shot_nz), irx) - tts(clip(irz - 1, 1, shot_nz), irx))/(2*dz)
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + tts_residual(j, 1)*dzt
                            end do
                            !$omp end parallel do
                            ...
                            !$omp parallel do private(j)
                            do j = 1, gmtr(ishot)%nr
                                model_grad(i)%array(j, 1) = model_grad(i)%array(j, 1) + ttp_residual(j, 1) + tts_residual(j, 1)
                            end do
                            !$omp end parallel do
    ...
    module subroutine compute_gradient_shots_tloc_dd(phase_label)
        ...
        ! Forward modeling to compute misfit; for DD where SR are exchanged, must first finish all forward modeling
        do ishot = shot_in_rank(rankid, 1), shot_in_rank(rankid, 2)
        ...
        do ishot = shot_in_rank(rankid, 1), shot_in_rank(rankid, 2)
            ...
            select case (which_medium)
                ...
                case ('acoustic-iso')
                    ...
                    ttp = load(tidy(dir_scratch)//'/shot_'//num2str(gmtr(ishot)%id)//'_traveltime_p.bin', nz, nx)
                ...
                case ('elastic-iso')
                    ...
                    ttp = load(tidy(dir_scratch)//'/shot_'//num2str(gmtr(ishot)%id)//'_traveltime_p.bin', nz, nx)
                    tts = load(tidy(dir_scratch)//'/shot_'//num2str(gmtr(ishot)%id)//'_traveltime_s.bin', nz, nx)

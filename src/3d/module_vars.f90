...
module vars
    ...
    type(meta_array3_real), allocatable, dimension(:) :: model_m, model_m_backup
    type(meta_array3_real), allocatable, dimension(:) :: model_grad, model_srch, model_reg
    type(meta_array3_real), allocatable, dimension(:) :: model_dws

contains

    !
    !> Allocate, input, crop, initialize a model
    !
            ...
            select case(name)
                ...
                case default
                    ...
                    if (present(file_w) .and. file_w /= '') then
                        ...
                        if (file_w == '') then
                            if (rankid == 0) then
                                call warn(' Warning: Model '//tidy(name)//' is empty and is set to '//num2str(w(1, 1, 1)))
                            end if
                        end if
                ...
                case ('sx', 'sy', 'sz', 'st0')
                    ...
                    if (present(file_w) .and. file_w /= '') then
                        ...
                        if (file_w == '') then
                            if (rankid == 0) then
                                call warn(' Warning: Source parameter '//tidy(name)//' is empty and is set to '//num2str(w(1, 1, 1)))
                            end if
                        end if
